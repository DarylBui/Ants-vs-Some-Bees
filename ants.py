...
class Place(object):
    ...
    def add_insect(self, insect):
        ...
        if insect.is_ant:
            if self.ant is None:
                ...
                self.ant = insect
            elif self.ant.is_container and self.ant.can_contain(insect) and not insect.is_container:
    ...
    def remove_insect(self, insect):
        ...
        if insect.is_ant:
            ...
            # BEGIN Problem 13
            if isinstance(insect, QueenAnt) and not insect.isimposter:
                return
            # END Problem 13
...
class Insect(object):
    ...
    def action(self, colony):
        ...
        return '{0}({1}, {2})'.format(self.name, self.armor, self.place)
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive):
        ...
        place = self.place
        distance = 0
        while not place == hive and place:
...
# END Problem 13
    ...
    def action(self, colony):
        ...
        if self.isimposter:
            ...
            while place:
                if not place.ant in self.buffedants and place.ant:
                    place.ant.damage *= 2
                    self.buffedants.append(place.ant)
                if place.ant and place.ant.is_container and place.ant.contained_ant and not place.ant.contained_ant in self.buffedants:
                        place.ant.contained_ant.damage *= 2
                        self.buffedants.append(place.ant.contained_ant)
                place = place.exit
        # END Problem 13
    ...
    def reduce_armor(self, amount):
        ...
        if self.armor <= 0 and not self.isimposter:
            bees_win()
        # END Problem 13
...
def make_scare(action, bee):
    ...
    """
    # BEGIN Problem EC
    def spoopy_action(colony):
        bee.direction = -1
        action(colony)
        bee.direction = 1
    return spoopy_action
    # END Problem EC
...
class ScaryThrower(ThrowerAnt):
    ...
    def throw_at(self, target):
        # BEGIN Problem EC
        if target and not target.spooped_yet:
            target.spooped_yet = True
